...
class ParserApp:
    ...
    # ---------- Инструкция: красивое оформление ----------
    def _build_instructions_tab(self, parent):
        """Строит вкладку Инструкция на CTkScrollableFrame с секциями и подсекциями."""
        ...
        def title(text):
            lbl = ctk.CTkLabel(
                scroll, text=text, font=ctk.CTkFont(size=22, weight="bold"),
                text_color=C_TITLE, anchor="w", justify="left",
            )
        ...
        def intro(text):
            lbl = ctk.CTkLabel(
                ...
                text_color="gray80", anchor="w", justify="left", wraplength=780,
            )
            lbl.pack(fill="x", padx=4, pady=(0, 14))

        def section(icon, text):
            frm = ctk.CTkFrame(scroll, fg_color="transparent")
            frm.pack(fill="x", padx=2, pady=(18, 4))
            ctk.CTkLabel(
                frm, text=f"{icon}  {text}",
                font=ctk.CTkFont(size=17, weight="bold"),
                text_color=C_SECTION, anchor="w", justify="left",
            ).pack(fill="x")
            sep = ctk.CTkFrame(scroll, fg_color=C_SECTION, height=2)
            sep.pack(fill="x", padx=2, pady=(0, 6))

        def subsection(text):
            ctk.CTkLabel(
                scroll, text=text, font=ctk.CTkFont(size=14, weight="bold"),
                text_color=C_SUB, anchor="w", justify="left",
            ).pack(fill="x", padx=10, pady=(10, 2))

        def step(num, text):
            frm = ctk.CTkFrame(scroll, fg_color="transparent")
            frm.pack(fill="x", padx=18, pady=1)
            ctk.CTkLabel(
                frm, text=f"{num}.", font=ctk.CTkFont(size=12, weight="bold"),
                text_color=C_SECTION, width=22, anchor="nw", justify="left",
            ).pack(side="left", anchor="n")
            ctk.CTkLabel(
                frm, text=text, font=ctk.CTkFont(size=12),
                text_color="white", anchor="w", justify="left", wraplength=720,
            ).pack(side="left", fill="x", expand=True)

        def bullet(text):
            frm = ctk.CTkFrame(scroll, fg_color="transparent")
            frm.pack(fill="x", padx=24, pady=1)
            ctk.CTkLabel(
                frm, text="•", font=ctk.CTkFont(size=12, weight="bold"),
                text_color=C_SECTION, width=14, anchor="nw",
            ).pack(side="left", anchor="n")
            ctk.CTkLabel(
                frm, text=text, font=ctk.CTkFont(size=12),
                text_color="white", anchor="w", justify="left", wraplength=720,
        ...
        def paragraph(text):
            ctk.CTkLabel(
                scroll, text=text, font=ctk.CTkFont(size=12),
                text_color="white", anchor="w", justify="left", wraplength=760,
            ).pack(fill="x", padx=18, pady=(4, 4))

        def note(text):
            box = ctk.CTkFrame(scroll, fg_color=C_NOTE_BG, corner_radius=6)
            box.pack(fill="x", padx=14, pady=(6, 6))
            ctk.CTkLabel(
                box, text=text, font=ctk.CTkFont(size=12, weight="bold"),
                text_color="#ffd984", anchor="w", justify="left", wraplength=720,
            ).pack(fill="x", padx=10, pady=6)

        def code(text):
            box = ctk.CTkFrame(scroll, fg_color=C_CODE_BG, corner_radius=6)
            box.pack(fill="x", padx=18, pady=(2, 6))
            ctk.CTkLabel(
                box, text=text, font=ctk.CTkFont(family="Consolas", size=12),
                text_color="#c9e7ff", anchor="w", justify="left",
            ).pack(fill="x", padx=10, pady=6)

        def link_button(label, url):
            btn = ctk.CTkButton(
                scroll, text=label, width=260, anchor="w",
        ...
        # -------- 1. Прокси --------
        section("🌐", "1. Прокси (на примере mobileproxy.space)")
        paragraph("Без прокси Авито быстро заблокирует твой IP. Мобильные прокси — самые устойчивые.")

        subsection("1.1. Покупка прокси")
        step(1, "Перейди на mobileproxy.space и выбери тариф.")
        bullet("Тип: HTTP или SOCKS5 (рекомендую HTTP).")
        bullet("Страна — только Россия. С иностранных IP Авито может показывать пустой список или капчу.")
        step(2, "После оплаты зайди в личный кабинет, раздел «Мои прокси».")
        link_button("🔗  Открыть mobileproxy.space", "https://mobileproxy.space/")

        subsection("1.2. Привязка IP-адреса")
        note("🔒  Обязательный шаг — без привязки IP прокси работать не будет.")
        step(1, "Найди купленный прокси в списке и отметь его галочкой.")
        step(2, "Нажми кнопку «Bind IP» (Привязать IP).")
        step(3, "В открывшемся окне кликни на ссылку со своим текущим IP — адрес подставится автоматически.")
        step(4, "Нажми «Сохранить».")

        subsection("1.3. Получение данных для подключения")
        paragraph("В личном кабинете найди параметры своего прокси:")
        bullet("Хост (например: z.mobilespace.proxy)")
        bullet("Порт (например: 12345)")
        bullet("Логин (если есть)")
        bullet("Пароль (если есть)")
        paragraph("Логин и пароль часто передаются одной строкой через двоеточие:")
        code("yR1ByZ:paNHYV8EM7su    # до двоеточия логин, после — пароль")

        subsection("1.4. Настройка в программе")
        step(1, "Открой вкладку «Настройки» → раздел «Прокси».")
        step(2, "Заполни поля: Тип, Хост, Порт, Логин/Пароль.")
        step(3, "Нажми «Тест прокси» — если всё ок, появится зелёная галочка.")

        # -------- 2. Telegram --------
        section("📬", "2. Telegram-уведомления")
        paragraph("Чтобы новые объявления приходили тебе в Telegram, нужны токен бота и твой Chat ID.")

        subsection("2.1. Создание бота")
        step(1, "Открой Telegram и найди @BotFather.")
        step(2, "Отправь ему /newbot и следуй подсказкам.")
        step(3, "Сохрани токен, который пришлёт BotFather — он понадобится в программе.")
        link_button("🔗  Открыть @BotFather", "https://t.me/BotFather")

        subsection("2.2. Получение Chat ID")
        step(1, "Напиши боту @WhatChatIDBot любое сообщение.")
        step(2, "Бот ответит твоим Chat ID — скопируй его.")
        link_button("🔗  Открыть @WhatChatIDBot", "https://t.me/WhatChatIDBot")

        subsection("2.3. Настройка в программе")
        step(1, "На вкладке «Настройки» → «Telegram уведомления» вставь токен и Chat ID.")
        step(2, "Нажми «Тест» — в Telegram должно прийти тестовое сообщение.")

        # -------- 3. Сохранение --------
        section("💾", "3. Сохрани настройки")
        paragraph("После заполнения всех полей нажми «💾 Запомнить настройки» — они сохранятся в settings.json.")

        # -------- Подвал --------
        section("🚀", "Всё готово")
        paragraph(
            "Возвращайся на вкладку «Поиск», задай запрос, город и диапазон цен — и жми «Запустить». "
            "Включи авто-обновление, чтобы программа сама проверяла новые объявления."
        )
    ...
    def _extract_seller_rating(self, item):
        ...
        selectors = [
            ...
            "xpath=.//span[contains(@class,'rating')]",
            "xpath=.//*[starts-with(@aria-label,'Рейтинг')]",
        ]
        for sel in selectors:
            try:
                ...
                if elem is None:
                    continue
                text_content = (elem.text_content() or "").strip()
                aria = elem.get_attribute("aria-label") or ""
                text = (text_content or aria).strip()
                if not text:
                    continue
                text = text.replace(",", ".")
    ...
    def run_parser(self, query, min_price, max_price, city):
        ...
        try:
            ...
            # Принимаем куки
            try:
                cookie_btn = driver.wait_for_selector(
                    '//button[contains(text(),"Принять")]', state='visible', timeout=5000)
                if cookie_btn:
                    cookie_btn.click()
            ...
            # Подтверждаем город
            try:
                city_btn = driver.wait_for_selector(
                    '//button[contains(text(),"Да")]', state='visible', timeout=5000)
                if city_btn:
                    city_btn.click()
            ...
            # Поиск
            try:
                search_input = driver.wait_for_selector(
                    "input[data-marker='search-form/suggest']", timeout=10000)
                if search_input:
                    search_input.fill(query)
                    search_button = driver.wait_for_selector(
                        "button[data-marker='search-form/submit-button']", state='visible', timeout=5000)
                    if search_button:
                        search_button.click()
            ...
            driver.wait_for_selector("[data-marker='item']", timeout=15000)
            ...
            if self.delivery_var.get():
                try:
                    ...
                    driver.evaluate("window.scrollBy(0, 300);")
                    random_sleep(0.7, 1.6)

                    delivery_element = None
                    selectors = [
                        "//span[contains(text(),'С Авито Доставкой')]",
                        "//label[contains(.,'С Авито Доставкой')]",
                    ]

                    for selector in selectors:
                        try:
                            elem = driver.wait_for_selector(selector, timeout=5000)
                            if elem is None:
                                continue
                            try:
                    ...
                    if delivery_element is None:
                        ...
                        try:
                            show_span = driver.wait_for_selector(
                                "//span[starts-with(text(),'Показать')]", timeout=10000)
                            if show_span is not None:
                                parent_button = show_span.query_selector("xpath=ancestor::button")
            ...
            last_height = driver.evaluate("document.body.scrollHeight")
            current_position = 0
            ...
            while attempts < max_scroll_attempts:
                ...
                driver.evaluate(f"window.scrollTo({{top: {current_position}, behavior: 'smooth'}});")
            ...
            items = driver.query_selector_all("[data-marker='item']")
    ...
    def extract_date(self, item):
        ...
        date_selectors = [
            ...
            "xpath=.//time",
            "xpath=.//*[contains(text(), 'сегодня') or contains(text(), 'вчера')]",
        ]
    ...
    def get_item_id(self, item):
        ...
        try:
            ...
            pass
        try:
            link_elem = item.query_selector("a[itemprop='url']")
            title_elem = item.query_selector("[itemprop='name']")
    ...
    def parse_items(self, items, min_price, max_price):
        ...
        for idx, item in enumerate(items):
            ...
            try:
                title_elem = item.query_selector("[itemprop='name']")
                title = (title_elem.text_content() or "").strip() if title_elem else "Н/Д"
                if not title:
                    title = "Н/Д"

                link_elem = item.query_selector("a[itemprop='url']")
                link = (link_elem.get_attribute("href") or "").strip() if link_elem else "Н/Д"
                if not link:
                    link = "Н/Д"
                ...
                price_elem = item.query_selector("[itemprop='price']")
                price = price_elem.get_attribute("content") if price_elem else None
                if not price:
                ...
                img_url = "Н/Д"
                img = item.query_selector("img[data-marker='image']")
                if img is not None:
                ...
                for desc_selector in [
                    ...
                    try:
                        ...
                        if desc is None:
                            continue
                        text = (desc.text_content() or "").strip()
    ...
    def send_telegram_notification(self, added):
        ...
        try:
            for c in self.driver_manager.cookies():
    ...
    def display_results(self):
        try:
            ...
            try:
                for cookie in self.driver_manager.cookies():
